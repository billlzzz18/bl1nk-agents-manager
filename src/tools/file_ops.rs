...
impl FileOpsTools {
    pub fn read_file(args: ReadFileArgs) -> Result<String> {
        let content =
            fs::read_to_string(&args.file_path).with_context(|| format!("Failed to read file: {}", args.file_path))?;
    ...
    pub fn replace(args: ReplaceArgs) -> Result<String> {
        ...
        if count > 1 && !args.allow_multiple.unwrap_or(false) {
            anyhow::bail!(
                "Found multiple occurrences of '{}'. Use allow_multiple=true to replace all.",
                args.old_string
            );
        }
